...
# login_user：调用user中的get_id方法获取用户ID，写到session的user_id里面
# login_required：判断当前用户是否登录，需要调用user中的is_authenticated()方法来判断用户是否登录
...
def before_app_request():
    ...
    if current_user.is_authenticated and \
        ...
        #注意：访问其他蓝本的时候，比如main蓝本时候，当前蓝本就是main：提示验证界面
